...
def _extract_tensor_info(t):
    """
    Extract flat data vector and shape tuple from any backend tensor.
    Compatible with pure Python, ctypes, and C extension backends.
    Returns: (vector_array, shape_tuple) where vector is array.array('d')
    """
    # Fast path: backends that expose internal attributes directly
    try:
        return array('d', t.vector), tuple(t.tensor_size)
    except AttributeError:
        pass

    # Fallback: infer shape by traversing first element of each dimension
    shape = []
    cur = t
    while True:
        try:
    ...
    # Flatten tensor to 1D array using index-based traversal (works with all backends)
    ndim = len(shape)
    total = 1
    for s in shape:
        total *= s
    vec = array('d', [0.0]) * total
    pos = 0

    def _flatten(obj, dim):
        nonlocal pos
        if dim == ndim:
            vec[pos] = obj
            pos += 1
            ...
            for i in range(len(obj)):
                _flatten(obj[i], dim + 1)

    _flatten(t, 0)
    return vec, tuple(shape)
