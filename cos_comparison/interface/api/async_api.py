...
__all__ = ("AsyncRunner", "EventLoop")


class EventLoop:
    """
    Periodic task host (mechanism only, no monitoring semantics).

    Schedules callables (sync or async) to be executed repeatedly at a fixed
    interval on an AsyncRunner event host. The periodic drive, sleeping and
    event management are implemented here (interface); higher layers supply
    the semantics of what each task does.
    """

    __slots__ = ("interval", "_host")

    def __init__(self, interval=0.01):
        self.interval = interval
        self._host = AsyncRunner()

    def schedule(self, task, interval=None):
        """
        Schedule ``task`` (sync or async callable) every ``interval`` seconds
        (defaults to the loop interval). Returns a handle usable with
        ``unschedule``.
        """
        period = self.interval if interval is None else interval
        ...
        return self._host.add_event(periodic)

    def unschedule(self, handle):
        """Thread-safely cancel a previously scheduled task."""
        return self._host.remove(handle)

    def run(self, *args, **kwargs):
        return self._host.run(*args, **kwargs)

    def stop(self, *args, **kwargs):
        return self._host.stop(*args, **kwargs)

    def is_running(self):
        return self._host.is_running()

    def wait(self, timeout=None):
        """Block until no events remain active (False on timeout)."""
        if timeout is None:
            while self._host.active():
                time.sleep(0.005)
            return True
