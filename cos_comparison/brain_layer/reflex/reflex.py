...
async def _trigger(obj,target,trigger:str,callback,sleep=0.01,times=-1):
    if type(times) != int:
...
class Trigger:
    ...
    async def _run(self):
        await asyncio.gather(*[_trigger(self.target,target,trigger,callback,t) for target,trigger,callback,t in self.callbacks])
    def run(self):
