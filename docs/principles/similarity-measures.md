...
Cos-comparison provides three complementary similarity algorithms, each capturing different aspects of the relationship between two vectors. All three follow a **universal form**: given the accumulated sums `a = Σ A²`, `b = Σ B²` and `ab = Σ A·B`, the similarity is `f(a, b, ab)`.
...
Where `a = ‖A‖²` (sum of squares of vector 1), `b = ‖B‖²` (sum of squares of vector 2), `ab = A·B` (dot product).
