...
- **Utility Functions**: `multiple_chain`, `add_chain`, `create_void_list`, `load_as_default_data`, `load_data` (per-side start/step, silent clipping, returns copied count; provided by pure Python and backfilled to compiled backends that do not ship it), `infer_shape`, `get_item` / `set_item`, `vector_chain_compute`, `no_done`
