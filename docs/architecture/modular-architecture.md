...
| `interface` | External abstraction | System commands & processes (`system_api`), thread/process locks and shared arrays (`parallel_api`), blocking async event host with thread-safe event injection (`async_api`), IO/Socket/Pipe/File communication (`communicate_api`), dynamic library & module calls (`call_api`), database access (`database_api`), context tools (`context_tool`) | Yes — stdlib only |
...
| `brain_layer` | Cognition layer | Symbolic logic, probability logic, reflex monitoring (Trigger / Monitor) | — |
