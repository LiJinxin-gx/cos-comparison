...
static PyObject* py_load_as_default_data(PyObject *self, PyObject *args, PyObject *kwargs) {
    PyObject *data_obj; int start = 0; PyObject *shape_obj = Py_None;
    static char *kwlist[] = {"data", "start", "shape", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|iO", kwlist, &data_obj, &start, &shape_obj))
        return NULL;
    int *shape = NULL; int dimension = 0;
    if (shape_obj == Py_None) {
        if (_infer_shape(data_obj, &shape, &dimension) < 0) return NULL;
        if (dimension == 0) { free(shape); PyErr_SetString(PyExc_ValueError, "cannot infer shape"); return NULL; }
    } else {
        if (!PySequence_Check(shape_obj)) { PyErr_SetString(PyExc_TypeError, "shape must be a sequence"); return NULL; }
        Py_ssize_t n = PySequence_Size(shape_obj);
        if (n == 0) { PyErr_SetString(PyExc_ValueError, "shape cannot be empty"); return NULL; }
        dimension = (int)n;
        ...
        for (int i = 0; i < dimension; ++i) {
            ...
            if (!num) { free(shape); return NULL; }
            shape[i] = (int)PyLong_AsLong(num); Py_DECREF(num);
            if (PyErr_Occurred()) { free(shape); return NULL; }
        }
    }
    Data *temp_data = Data_create(dimension, shape); free(shape);
    if (!temp_data) { PyErr_NoMemory(); return NULL; }
    int idx = 0;
    if (_flatten_list(data_obj, temp_data->data, &idx, dimension, temp_data->shape) < 0) {
        Data_free(temp_data); return NULL;
    }
    return _data_to_independent_vector(temp_data, start);
}
