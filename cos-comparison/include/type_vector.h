...
static int _flatten_list_to_data(PyObject *obj, double *out, int *idx, int dim, const int *shape) {
    ...
    for (Py_ssize_t i = 0; i < len; ++i) {
        ...
        if (!item) return -1;
        if (_flatten_list_to_data(item, out, idx, dim - 1, shape + 1) < 0) {
            Py_DECREF(item);
            return -1;
        }
        Py_DECREF(item);
...
static int Vector_init(Vector *self, PyObject *args, PyObject *kwargs) {
    ...
    /* Case 3: infer shape and copy data into a new double array */
    int *shape = NULL;
    int dim = 0;
    if (_infer_shape(vector, &shape, &dim) < 0) return -1;
    if (dim <= 0) {
        free(shape);
