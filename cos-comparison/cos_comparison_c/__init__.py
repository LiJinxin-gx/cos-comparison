...
def _data_to_list(data):
    ...
    def build(dim_idx, offset):
        ...
        sub = []
        for i in range(size):
            sub.append(build(dim_idx+1, offset + i*stride))
        return sub
    return build(0, 0)
...
def _copy_nested_with_step(src, dst, start=None, step=None, dim=0):
    if isinstance(src, list):
        for i in range(len(src)):
            idx = (start[dim] if start else 0) + (step[dim] if step else 1) * i
            if isinstance(src[i], list):
                _copy_nested_with_step(src[i], dst[idx], start, step, dim+1)
            else:
                dst[idx] = src[i]
